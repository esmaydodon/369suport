<template>
  <div v-loading="loading">
    <el-form ref="formVinculoLaboral" :model="historiaClinica" :rules="reglas" label-position="top">
      <el-row :gutter="10">
        <el-col :span="24">
...
export default {
  name: 'AgregarEditarVinculoLaboral',
  props: {
    historiaClinicaId: {
      required: true,
      type: Number
  ...
  methods: {
    ...
    handleSubmitForm() {
      this.$refs['formVinculoLaboral'].validate((valid) => {
        if (valid) {
          if (this.historiaClinicaId < 0) {
    ...
    close() {
      ...
      this.$refs['formVinculoLaboral'].resetFields()
      this.$emit('close')
    }
  }
}
